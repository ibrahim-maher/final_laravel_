                ...
                <tr data-ride-id="{{ $rideId }}" class="hover:bg-gray-50">
                    ...
                    <td class="px-6 py-4">
                        <div class="flex items-center gap-2">
                            <a href="{{ route('admin.rides.show', $rideId) }}"
...
<script>
    ...
    async function updateRideStatus(rideId, status) {
        ...
        try {
            // Fix: Build the URL properly by replacing the placeholder
            const baseUrl = "{{ route('admin.rides.update-status', 'RIDE_ID_PLACEHOLDER') }}";
            const url = baseUrl.replace('RIDE_ID_PLACEHOLDER', rideId);
    ...
    async function completeRide(rideId) {
        ...
        try {
            // Fix: Build the URL properly
            const baseUrl = "{{ route('admin.rides.complete', 'RIDE_ID_PLACEHOLDER') }}";
            const url = baseUrl.replace('RIDE_ID_PLACEHOLDER', rideId);
    ...
    async function cancelRide(rideId) {
        ...
        try {
            // Fix: Build the URL properly
            const baseUrl = "{{ route('admin.rides.cancel', 'RIDE_ID_PLACEHOLDER') }}";
            const url = baseUrl.replace('RIDE_ID_PLACEHOLDER', rideId);
    ...
    async function exportRides(format) {
        try {
            const baseUrl = "{{ route('admin.rides.export') }}";
