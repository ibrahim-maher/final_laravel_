        ...
        <div class="flex items-center">
            ...
            <div>
                ...
                <p class="text-2xl font-bold text-gray-900">
                    {{ isset($statistics['active_vehicles']) ? $statistics['active_vehicles'] : 0 }}
                </p>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-lg shadow-sm border p-6">
        <div class="flex items-center">
            ...
            <div>
                ...
                <p class="text-2xl font-bold text-gray-900">
                    {{ isset($statistics['verified_vehicles']) ? $statistics['verified_vehicles'] : 0 }}
                </p>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-lg shadow-sm border p-6">
        <div class="flex items-center">
            ...
            <div>
                ...
                <p class="text-2xl font-bold text-gray-900">
                    {{ isset($statistics['pending_verification']) ? $statistics['pending_verification'] : 0 }}
                </p>
            </div>
        </div>
    </div>
    ...
    <form method="GET" action="{{ route('admin.vehicles.index') }}" class="flex flex-wrap gap-4">
        ...
        <div class="min-w-48">
            <select name="vehicle_type" class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-primary">
                ...
                <option value="hatchback" {{ ($vehicle_type ?? '') === 'hatchback' ? 'selected' : '' }}>Hatchback</option>
        ...
        <div class="min-w-32">
            <select name="limit" class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-primary">
                <option value="15" {{ ($limit ?? 15) == 15 ? 'selected' : '' }}>15</option>
                <option value="25" {{ ($limit ?? 15) == 25 ? 'selected' : '' }}>25</option>
                <option value="50" {{ ($limit ?? 15) == 50 ? 'selected' : '' }}>50</option>
            </select>
        </div>
    ...
    <div class="overflow-x-auto">
        <table class="w-full">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                ...
                <tr data-vehicle-id="{{ $vehicle->id }}" class="hover:bg-gray-50">
                    <td class="px-6 py-4">
                        <input type="checkbox" class="vehicle-checkbox rounded border-gray-300 text-primary focus:ring-primary"
                            value="{{ $vehicle->id }}" onchange="updateBulkActions()">
                    </td>
                    <td class="px-6 py-4">
                        <div class="flex items-center">
                            <div class="flex-shrink-0 mr-3">
                                @switch($vehicle->vehicle_type ?? 'car')
                                @case('sedan')
                            ...
                            <div>
                                ...
                                <div class="text-xs text-gray-400">ID: {{ Str::limit($vehicle->id, 12) }}</div>
                            </div>
                        </div>
                    </td>
                    <td class="px-6 py-4">
                        <div class="text-sm text-gray-900">{{ $vehicle->driver->name ?? 'Unknown Driver' }}</div>
                        <div class="text-sm text-gray-500">{{ $vehicle->driver->email ?? 'No Email' }}</div>
                    </td>
                    <td class="px-6 py-4">
                        <div class="text-sm space-y-1">
                            <div><span class="font-medium text-gray-700">Type:</span> {{ ucfirst($vehicle->vehicle_type ?? 'unknown') }}</div>
                            <div><span class="font-medium text-gray-700">Color:</span> {{ $vehicle->color ?? 'N/A' }}</div>
                            @if($vehicle->seats)
                    ...
                    <td class="px-6 py-4 text-sm text-gray-900">
                        <div>{{ $vehicle->created_at ? $vehicle->created_at->format('M d, Y') : 'Unknown' }}</div>
                        <div class="text-xs text-gray-500">{{ $vehicle->created_at ? $vehicle->created_at->diffForHumans() : 'Date not available' }}</div>
                        @if($vehicle->vin)
                        <div class="text-xs text-gray-500">VIN: {{ Str::substr($vehicle->vin, -4) }}</div>
                        @endif
                    </td>
                    <td class="px-6 py-4">
                        <div class="flex items-center gap-2">
                            ...
                            @if(($vehicle->verification_status ?? 'pending') !== 'verified')
                            <button onclick="updateVerificationStatus('{{ $vehicle->id }}', 'verified')"
                                class="text-blue-600 hover:text-blue-800 p-1" title="Verify Vehicle">
                                <i class="fas fa-shield-check"></i>
                            </button>
                            @endif
                            <button onclick="deleteVehicle('{{ $vehicle->id }}', '{{ addslashes(($vehicle->year ?? '') . ' ' . ($vehicle->make ?? '') . ' ' . ($vehicle->model ?? '')) }}')"
...
<script>
    ...
    async function updateVerificationStatus(vehicleId, status) {
        ...
        try {
            const response = await fetch(`{{ url('admin/vehicles') }}/${vehicleId}/verification`, {
                method: 'POST',
                headers: {
    ...
    async function deleteVehicle(vehicleId, vehicleName) {
        ...
        try {
            const response = await fetch(`{{ url('admin/vehicles') }}/${vehicleId}`, {
                method: 'DELETE',
                headers: {
                    'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content,
                    'Accept': 'application/json'
