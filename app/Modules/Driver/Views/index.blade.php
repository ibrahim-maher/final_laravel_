...
<div class="flex justify-between items-center mb-6">
    ...
    <div class="flex gap-3">
        ...
        <a href="{{ route('driver.statistics') }}" 
...
<div class="bg-white rounded-lg shadow-sm border p-6 mb-6">
    <form method="GET" action="{{ route('driver.index') }}" class="flex flex-wrap gap-4">
        ...
        <div class="flex gap-2">
            ...
            <a href="{{ route('driver.index') }}" class="bg-gray-500 text-white px-6 py-2 rounded-lg hover:bg-gray-600 transition-colors">
...
<div class="bg-white rounded-lg shadow-sm border overflow-hidden">
                ...
                <tr data-driver-id="{{ $driverId }}" class="hover:bg-gray-50">
                    ...
                    <td class="px-6 py-4">
                        <div class="flex items-center gap-2">
                            <a href="{{ route('driver.show', $driverId) }}" 
                               class="text-primary hover:text-blue-700 p-1" title="View Details">
                                <i class="fas fa-eye"></i>
                            </a>
                            <a href="{{ route('driver.edit', $driverId) }}" 
    ...
    <div class="text-center py-12">
        ...
        <div class="flex justify-center gap-3">
            <a href="{{ route('driver.create') }}" class="bg-primary text-white px-4 py-2 rounded-lg hover:bg-blue-700">
...
<script>
    ...
    async function deleteDriver(driverId, driverName) {
        ...
        try {
            const response = await fetch(`{{ route('driver.destroy', '') }}/${driverId}`, {
                method: 'DELETE',
                headers: {
                    'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content,
                    'Accept': 'application/json'
    ...
    async function bulkAction(action) {
        ...
        try {
            const response = await fetch('{{ route("driver.bulk-action") }}', {
                method: 'POST',
                headers: {
                    'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content,
                    'Content-Type': 'application/json',
    ...
    async function exportDrivers(format) {
        try {
            const url = new URL('{{ route("driver.export") }}');
