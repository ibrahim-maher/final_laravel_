<?php
// app/Modules/TaxSetting/Models/TaxSetting.php

namespace App\Modules\TaxSetting\Models;
...
class TaxSetting extends Model
{
    ...
    const TAX_TYPE_HYBRID = 'hybrid';

    const CALCULATION_SIMPLE = 'simple';
    const CALCULATION_COMPOUND = 'compound';
    const CALCULATION_CASCADING = 'cascading';
    ...
    {
        if ($this->tax_type === self::TAX_TYPE_PERCENTAGE) {
            return $this->rate . '%';
        } elseif ($this->tax_type === self::TAX_TYPE_FIXED) {
    ...
    public function calculateTax($amount, $context = [])
    {
        if (!$this->is_valid || $amount < ($this->minimum_taxable_amount ?? 0)) {
            return 0;
        }
        ...
        switch ($this->tax_type) {
            case self::TAX_TYPE_PERCENTAGE:
                $taxAmount = ($amount * $this->rate) / 100;
                break;
            case self::TAX_TYPE_FIXED:
                $taxAmount = $this->fixed_amount;
                break;
            case self::TAX_TYPE_HYBRID:
                $taxAmount = ($amount * $this->rate) / 100 + $this->fixed_amount;
                break;
        }

        // Apply maximum tax limit if set
        if ($this->maximum_tax_amount && $taxAmount > $this->maximum_tax_amount) {
            $taxAmount = $this->maximum_tax_amount;
        }

        return round($taxAmount, 2);
    }

    // Check if tax applies to context
    ...
    public function toFirebaseArray()
    {
        return [
            ...
            'tax_type' => $this->tax_type ?? 'percentage',
            'calculation_method' => $this->calculation_method ?? 'simple',
            'rate' => (float) ($this->rate ?? 0),
            'fixed_amount' => (float) ($this->fixed_amount ?? 0),
            'minimum_taxable_amount' => (float) ($this->minimum_taxable_amount ?? 0),
            'maximum_tax_amount' => $this->maximum_tax_amount ? (float) $this->maximum_tax_amount : null,
            ...
            'is_inclusive' => (bool) ($this->is_inclusive ?? false),
    ...
    private function getCleanArray($fieldName)
    {
        try {
            ...
            Log::warning("Error processing array field {$fieldName} for tax setting {$this->id}: " . $e->getMessage());
            return [];
        }
    }
    ...
    public static function getTaxTypes()
    {
        return [
            ...
            self::TAX_TYPE_FIXED => 'Fixed Amount',
            self::TAX_TYPE_HYBRID => 'Hybrid (Percentage + Fixed)'
        ];
    }
    ...
    public static function getCalculationMethods()
    {
        return [
            ...
            self::CALCULATION_COMPOUND => 'Compound',
            self::CALCULATION_CASCADING => 'Cascading'
        ];
    }
    ...
    protected static function boot()
    {
        ...
        static::saving(function ($taxSetting) {
            ...
            if (!$taxSetting->priority_order) {
                $maxPriority = static::max('priority_order') ?? 0;
                $taxSetting->priority_order = $maxPriority + 1;
            }

            // Clean array fields
            $arrayFields = [
                'applicable_zones', 'excluded_zones', 'applicable_vehicle_types', 
                'excluded_vehicle_types', 'applicable_services', 'excluded_services'
            ];

            foreach ($arrayFields as $field) {
                if ($taxSetting->isDirty($field)) {
                    $value = $taxSetting->getAttribute($field);
                    
                    if (is_null($value) || $value === '' || $value === 'null') {
                        $taxSetting->setAttribute($field, []);
                    } elseif (is_string($value)) {
                        $decoded = json_decode($value, true);
                        if (json_last_error() === JSON_ERROR_NONE && is_array($decoded)) {
                            $taxSetting->setAttribute($field, array_values(array_filter($decoded)));
                        } else {
                            $csvArray = array_map('trim', explode(',', $value));
                            $taxSetting->setAttribute($field, array_values(array_filter($csvArray)));
                        }
                    } elseif (is_array($value)) {
                        $taxSetting->setAttribute($field, array_values(array_filter($value)));
                    } else {
                        $taxSetting->setAttribute($field, []);
                    }
                }
